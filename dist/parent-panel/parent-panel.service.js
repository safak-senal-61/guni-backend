...
exports.ParentPanelService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../prisma/prisma.service");
const user_roles_enum_1 = require("../common/enums/user-roles.enum");
let ParentPanelService = class ParentPanelService {
    prisma;
    constructor(prisma) {
        this.prisma = prisma;
    }
    async getParentProfile(parentId) {
        ...
        if (!parent) {
            throw new common_1.NotFoundException('Veli bulunamadı');
        }
        return {
            id: parent.id,
    ...
    async requestStudentConnection(parentId, connectDto) {
        const student = await this.prisma.user.findUnique({
            where: { email: connectDto.studentEmail }
        });
        if (!student) {
            throw new common_1.NotFoundException('Bu email adresine sahip öğrenci bulunamadı');
        }
        if (student.role !== user_roles_enum_1.UserRole.STUDENT) {
            throw new common_1.BadRequestException('Bu kullanıcı bir öğrenci değil');
        }
        ...
        if (existingConnection) {
            throw new common_1.BadRequestException('Bu öğrenci ile zaten bir bağlantı mevcut');
        }
    ...
    async getPendingConnections(parentId) {
    ...
    async getConnectedStudents(parentId) {
        const connections = await this.prisma.parentStudentConnection.findMany({
            where: {
                parentId,
    ...
    async getStudentDetailedProgress(parentId, dto) {
        ...
        if (!connection) {
            throw new common_1.ForbiddenException('Bu öğrencinin verilerine erişim yetkiniz yok');
        }
        ...
        return {
            ...
            recentLessons: student?.lessonsCompleted?.map(lp => ({
                ...
                timeSpent: Math.round(lp.timeSpent / 60),
            ...
            recentQuizzes: student?.quizzesTaken?.map(qa => ({
                ...
                timeTaken: qa.timeTaken ? Math.round(qa.timeTaken / 60) : null,
    ...
    async sendNotificationToStudent(parentId, dto) {
        const connection = await this.prisma.parentStudentConnection.findUnique({
            where: {
                parentId_studentId: {
                    parentId,
        ...
        if (!connection) {
            throw new common_1.ForbiddenException('Bu öğrenciye bildirim gönderme yetkiniz yok');
        }
    ...
    async markNotificationAsRead(parentId, notificationId) {
        ...
        if (notification.receiverId !== parentId) {
            throw new common_1.ForbiddenException('Bu bildirimi okuma yetkiniz yok');
        }
    ...
    async generateWeeklyProgressSummary(parentId, studentId) {
        const connection = await this.prisma.parentStudentConnection.findUnique({
            where: {
                parentId_studentId: {
                    parentId,
        ...
        const summaryData = {
            ...
            averageScore,
            totalStudyTime: Math.round((totalStudyTime._sum.timeSpent || 0) / 60),
            weakSubjects,
            strongSubjects,
    ...
    async createWeeklyProgressSummary(parentId, studentId) {
        ...
        if (!connection) {
            throw new common_1.ForbiddenException('No approved connection found with this student');
        }
    ...
    async getStudentAnalytics(parentId, studentId) {
        ...
        for (let i = 7; i >= 0; i--) {
            ...
            const weekLessons = lessonProgress.filter(lp => lp.lastAccessed >= weekStart && lp.lastAccessed < weekEnd && lp.completed);
            const weekQuizzes = quizAttempts.filter(qa => qa.attemptDate >= weekStart && qa.attemptDate < weekEnd);
            weeklyProgress.push({
        ...
        const totalTime = Object.values(timeStats).reduce((sum, time) => sum + time, 0);
        const timeDistribution = Object.entries(timeStats).map(([subject, seconds]) => ({
            subject,
    ...
    async getDashboardSummary(parentId) {
        const connections = await this.prisma.parentStudentConnection.findMany({
            where: {
                parentId,
                connectionStatus: 'approved',
    ...
    async getStudentSchedule(parentId, studentId) {
        const connection = await this.prisma.parentStudentConnection.findFirst({
            where: {
                parentId,
                studentId,
    ...
    async setStudyGoals(parentId, studentId, goals) {
        const connection = await this.prisma.parentStudentConnection.findFirst({
            where: {
                parentId,
                studentId,
...
exports.ParentPanelService = ParentPanelService;
exports.ParentPanelService = ParentPanelService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], ParentPanelService);
//# sourceMappingURL=parent-panel.service.js.map